...
async def download_itinerary_pdf_async(location: str, days: int, start_date: str, preferences: Optional[dict] = None):
    """
...
async def forward_chat_async(message: str) -> Dict[str, Any]:
    """
    ...
    logger.info("Forwarding chat message to ML model travel assistant.")
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.error(f"Error forwarding chat to ML model: {e}")
            return {"reply": "Sorry, I'm having trouble connecting. Please ensure the ML model is running."}
